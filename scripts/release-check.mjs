...
const requestedVersion = versionArgIndex >= 0 ? process.argv[versionArgIndex + 1] : undefined;
...
const manifests = packageDirs.map((dir) => ({ dir, manifest: readJson(join(dir, 'package.json')) }));
...
if (publicVersions.size > 1) {
  failures.push(`Publishable npm packages use inconsistent versions: ${[...publicVersions].join(', ')}.`);
}
const npmVersion = [...publicVersions][0];
if (requestedVersion && npmVersion && requestedVersion !== npmVersion) {
  failures.push(`Release version ${requestedVersion} does not match npm package version ${npmVersion}.`);
}
...
for (const dir of pythonPackages) {
  ...
  if (!match) failures.push(`${dir}/pyproject.toml has no project version.`);
  else pythonVersions.add(match[1]);
}
if (pythonVersions.size > 1) {
  warnings.push(`Python package versions differ (expected for separate distributions): ${[...pythonVersions].join(', ')}.`);
}
try {
  execFileSync('python3', ['-m', 'build', '--version'], { cwd: root, stdio: 'ignore' });
} catch {
  warnings.push('python3 -m build is unavailable; the tagged workflow installs it before building Python artifacts.');
}
...
if (failures.length) {
  ...
  console.log(`release-check passed (${publicPackages.length} npm packages inspected; no publication performed).`);
}
